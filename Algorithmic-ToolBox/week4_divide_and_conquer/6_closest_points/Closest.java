...
public class Closest {
    ...
    static double stripMin(List<Point> strip, int stripLength, double d) {
        ...
        // if some points in array not initialized => they're null then Arrays.sort
        // wouldnt work

        // SORT BY Y SO we so there are at most 6 points for each Point that satisfy the
        // search dist < d
        strip = strip.stream().filter(i -> i != null).sorted((p1, p2) -> Long.compare(p1.y, p2.y))
    ...
    static double minDist(Point[] points, int left, int right) {
        ...
        // Trivial -> in this case we are minimizing so Return a value that can never be
        // the answer -> INFINITY
        if (left >= right)
        ...
        d = Math.min(d1, d2);
        // Create Strip where points are at most d from midpoint as only these can have
